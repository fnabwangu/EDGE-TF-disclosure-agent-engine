"""
EDGE-TF Disclosure Agent Engine - Indicative Intra-Day Value (IIV / INAV) Calculator.

Path: analytics/inav_calculator.py
...
class INAVCalculator:
    """
    Computes real-time Indicative Intra-Day Value (IIV / INAV) for ETF shares
    ...
    def calculate_inav(
        ...
        if etf_secondary_market_price and etf_secondary_market_price > 0 and inav_per_share > 0:
            spread = etf_secondary_market_price - inav_per_share
            premium_discount_bps = round((spread / inav_per_share) * 10_000, 2)
            if abs(premium_discount_bps) >= self.dislocation_threshold_bps:
                dislocation_flag = True
                logging.warning(
                    f"INAV Dislocation Alert: ETF trading at {premium_discount_bps:+.1f} bps "
                    f"(Market: ${etf_secondary_market_price:.2f}, INAV: ${inav_per_share:.2f})."
                )

        return INAVSnapshot(
            timestamp_utc=now_ts,
            indicative_nav_per_share=round(inav_per_share, 4),
            ...
            details={
                ...
                "creation_unit_inav": round(inav_per_share * self.creation_unit_size, 2),
            },
        )
